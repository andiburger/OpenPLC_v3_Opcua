...
if [ "$OPENPLC_PLATFORM" = "win" ]; then
    ...
    ./glue_generator
    echo "Compiling main program..."
    g++ -std=gnu++11 *.cpp *.o -o openplc -I ./lib -pthread -fpermissive `pkg-config --cflags --libs libmodbus` -L /workdir/open62541/build/bin -lasiodnp3 -lasiopal -lopendnp3 -lopenpal -lopen62541 -w $ETHERCAT_INC
    if [ $? -ne 0 ]; then
        echo "Error compiling C files"
        echo "Compilation finished with errors!"
        exit 1
